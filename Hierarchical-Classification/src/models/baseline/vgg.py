...
class VGG(nn.Module):
    ...
    def __init__(self, features, class_size=1000, if_classify = True, init_weights=True):
        super(VGG, self).__init__()
        self.features = features
        self.classifier = ClassifierCell(class_size)
    ...
    def forward(self, input, protocol):
        ...
        if(self.if_classify):
            ...
            x = self.features(x)
            x = self.classifier(x)
            loss = tuning_param['classification']*self.classification_loss_fn(x, label)
