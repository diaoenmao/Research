...
def make_dataset(dir, extensions, classes_to_labels=None):
    images = []
        ...
        for target in sorted(classes_to_labels.keys()):
            ...
            for root, _, fnames in sorted(os.walk(d)):
                for fname in sorted(fnames):
                    if has_file_allowed_extension(fname, extensions):
                        path = os.path.join(root, fname)
                        item = (path, classes_to_labels[target])
                        images.append(item)
    return images

def merge_classes(samples, class_map):
    for s in samples:
        c = str(s[1])
        if(c in class_map):
            s[1] = class_map[c] 
...
def make_classes_counts(samples,class_sizes):
    classes_counts = torch.zeros(class_sizes)
    for s in samples:
        classes_counts[s[1]] += 1
