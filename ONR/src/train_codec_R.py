...
def runExperiment(Experiment_TAG):
    ...
    for epoch in range(last_epoch, validated_num_epochs+1):
        ...
        if(save_mode>1):
            save({'epoch':epoch+1,'model_dict':model.state_dict(),'optimizer_dict':optimizer.state_dict(),'scheduler_dict': scheduler.state_dict()},'./output/model/checkpoint_{}.pkl'.format(Experiment_TAG))
            if(best_pivot>test_result[2].avg):
                best_pivot = test_result[2].avg if best_pivot>test_result[2].avg else best_pivot
                save({'epoch':epoch+1,'model_dict':model.state_dict(),'optimizer_dict':optimizer.state_dict(),'scheduler_dict': scheduler.state_dict()},'./output/model/best_{}.pkl'.format(Experiment_TAG))
    ...
    if(save_mode>0):
        save({'model_dict':model.state_dict(),'optimizer_dict':optimizer.state_dict()},'./output/model/{}.pkl'.format(Experiment_TAG))  
...
def resume(model,optimizer,scheduler,Experiment_TAG):
    if(os.path.exists('./output/model/checkpoint_{}.pkl'.format(Experiment_TAG))):
        checkpoint = load('./output/model/checkpoint_{}.pkl'.format(Experiment_TAG))
