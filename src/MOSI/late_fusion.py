...
def runExperiment(seed,Experiment_TAG):
    ...
    modality_valid_acc = np.zeros(3)
    for i in range(len(modality_name)):
        model_weight[i] = train_modality(train_loader[i],valid_loader[i],TAG+modality_name[i])
    model_weight = model_weight/np.sum(model_weight)
    for i in range(len(modality_name)):
        train_modality(eval_loader[i],test_loader[i],TAG+modality_name[i])
    model_average(test_loader,model_weight,TAG+modality_name[i])
    if(if_show):
        plt_result(TAG+modality_name[i])
    return

def model_average(test_loader,model_weight,modality_name,TAG):
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            best = torch.load('./output/model/best_{}.pth'.format(TAG))
...
def train_modality(train_loader,test_loader,TAG):
    criterion = nn.CrossEntropyLoss().to(device)
    model = eval('models.{}.{}(input_feature={},output_feature={}).to(device)'.format(model_dir,model_name,input_feature,output_feature))
    mw = modelWrapper(model,config.PARAM['optimizer_name'])
    mw.set_optimizer_param(config.PARAM['optimizer_param'])
