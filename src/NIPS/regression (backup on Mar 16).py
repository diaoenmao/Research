...
#    np.random.seed(0)
    ...
    beta = 10.0 / np.power(range(1,100+1),2)
    y = X.dot( beta.reshape([100,1]) ) + np.random.randn(dataSize,1)
    return X, y
...
#%% pre-def the observation up to time T 
    ...
    for t in range(t_start-1,T): #for each sample size 
        ...
        subE = np.array([ (candModels_fix[i]['lossEst'] + 2.0/t*len(candModels_fix[i]['var'])) for i in actSet]).reshape(K,) #experts
...
def LogisticFit(candModels, Xt,  yt):
    ...
    for n in range(N):
        ...
        if len(var) > len(yt):
            ...
            candModels[n]['lossEst'] = getTICLoss(Xt[:,var], yt, beta) #TIC adjusted loss
    return candModels

def getTICLoss(X, y, beta): #get TIC adjusted loss
    mu = X.dot( beta )
...
#    print 'X', X
#    print 'V', V
    pen = np.trace( lin.inv(V).dot(J) ) / n 
    #pen = 1.0 * m / n
    #print(pen) 
    return -loglik + pen 
...
def calculate_LogisticLoss(candModels, X_test, y_test):
    ...
    for n in range(N):
        var, beta = candModels[n]['var'], candModels[n]['beta']
        if beta is not None:
            mu = X_test[:,var].dot( beta )
            loss[n] = -(np.sum(mu[y_test==1]) - np.sum( np.log(1+np.exp(mu)) ))
        else:
            loss[n] = -np.inf
...
#%% pre-def the observation up to time T 
    ...
    t_start = 11 #starting data size
    nJumpExpe = 1*np.sqrt(T)/K #expected #jump 
    learningParams = {'eta':0.1*np.sqrt(8*nJumpExpe/T), 'alpha':nJumpExpe/T, 'G':None} #will use getSeqEdvice()
    actSet, move, thresh = range(K), False, 0.3 #input to learning
    subW = np.zeros((K))
    subW[0] = 1    
...
#        print move
    ...
    L[L==-np.inf] = np.max(L) #the none are set to be the max loss
    viewLoss(np.log(L), actSet_start, actSet_end) #viewLoss(L)
    viewSeqWeight(W_hy, np.log(L)) #print subW
    viewSeqLoss(np.log(seqPredLoss), np.log(L), t_start)
    #viewSeqLoss(seqPredLoss, L, t_start)
    #videoLoss(np.log(L), actSet_start, actSet_end)
    #videoSeqWeight(W_hy, L)
