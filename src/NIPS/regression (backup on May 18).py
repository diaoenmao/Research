...
#fit linear model at time t
def LogisticFit(candModels, Xt,  yt,  timing):
    N = len(candModels)
    kmax = 0
    Loss_AIC = np.inf
    for n in range(N):
        ...
        if len(var) > len(yt):
            ...
            if(n<np.floor(np.sqrt(Xt.shape[0])).astype(np.int) or n < 3):
                #Holdout
                start=time.time()
                X_train, X_test, y_train, y_test=crossvalition_p(Xt[:,var], yt, 0.7)
                ...
                #CV_10fold
                start=time.time()            
                X_train, X_test, y_train, y_test=crossvalition_10fold(Xt[:,var], yt)
                ...
                #CV_loo        
                start=time.time()             
                X_train, X_test, y_train, y_test=crossvalition_loo(Xt[:,var], yt)
                Loss=np.zeros(len(X_train))
                for i in range(len(X_train)):
                    logistic.fit(X_train[i], y_train[i])
                    beta = logistic.coef_.reshape([len(var),1])
            ...
            beta = logistic.coef_.reshape([len(var),1])
            bias = logistic.intercept_
            Loss = getLoss(Xt[:,var], yt, beta, bias)
            ...
            if(n<np.floor(np.sqrt(Xt.shape[0])).astype(np.int) or n < 3):            
                ...
                start_TIC=time.time()            
                TIC = getTIC(Xt[:,var], yt, beta, bias) 
                print(Loss)
                print(TIC)
...
#%% pre-def the observation up to time T 
    ...
    for t in range(t_start-1,T): #for each sample size
        ...
        Xt = X[range(t),:]
        yt = y[range(t),0]
        candModels_fix,timing = LogisticFit(candModels_fix, Xt, yt, timing)
    ...
    for t in range(t_start-1,T): #for each sample size
        ...
        for m in range(len(mode)):
            ...
            if candModels_Sequential[m]['move']:
                ...
                if max(candModels_Sequential[m]['actSet']) >= cur_dT:
                    candModels_Sequential[m]['actSet'] = range(cur_dT-K, cur_dT)
