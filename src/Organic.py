...
class Organic_info:        
    def __init__(self,p,if_collapse):
        self.p = [p.to('cpu')]
        self.if_collapse = if_collapse
...
def gibbs_organic(input,target,mw,m):
    for i in range(m.in_channels):
        ...
        cur_z = m.z
        cur_z[:,i] = 1
        output_1 = mw.model(input)
        likelihood_1 = torch.exp(-mw.loss(output_1,target))
        cur_z[:,i] = 0
...
def genetic_organic(input,target,mw,m):
